...
const StatCard002 = ({stat, classes}: StatCardProps): JSX.Element => {
  const value = getAttr(stat, "value") as string;
  const label = getAttr(stat, "label") as string;
  const background = getAttr(stat, "background") as string;
  ...
  return (
    <div className={rootClass} style={{backgroundColor: background}}>
...
export default React.memo(StatCard002);
